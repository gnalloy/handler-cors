...
Gnalloy HTTP/1 CORS Pipeline Handler，支持 preflight 与 credential-safe wildcard 行为。

该模块提供 Pipeline handler。handler 在 Channel 已存在之后观察、转换、拒绝、延迟、记录或保护消息；除非模块名明确说明，否则不拥有监听 socket 或应用协议。

## 状态

- 导入路径：`gnalloy.org/handler-cors`
- 仓库：`github.com/gnalloy/handler-cors`
- 默认分支：`dev`
- 预览安装：`go get gnalloy.org/handler-cors@dev`
- 许可证：Apache-2.0

## 安装
```bash
go get gnalloy.org/handler-cors@dev
go doc gnalloy.org/handler-cors
...
本仓库负责：Gnalloy HTTP/1 CORS Pipeline Handler，支持 preflight 与 credential-safe wildcard 行为。
