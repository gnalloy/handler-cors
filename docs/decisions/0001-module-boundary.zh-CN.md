# ADR-001：保持 handler-cors 为独立 Gnalloy 模块
...
`handler-cors` 作为 `gnalloy.org/handler-cors` 发布，并拥有该职责：Gnalloy HTTP/1 CORS Pipeline Handler，支持 preflight 与 credential-safe wildcard 行为。
